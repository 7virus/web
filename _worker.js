...
async function handleRequest(request) {
  ...
  switch (action) {
    case "generate":
      return await generateUUID(url.searchParams.get("proxy"));
    case "validate":
      return await validateUUID(url.searchParams.get("uuid"));
    default:
      return new Response("Invalid action!", { status: 400 });
  }
}

// Generate UUID and store it in KV Storage
async function generateUUID(proxy) {
  if (!proxy) {
    return new Response("Proxy is missing!", { status: 400 });
  }

  const uuid = crypto.randomUUID(); // Generate UUID
  const expiry = Math.floor(Date.now() / 1000) + 3600; // 1 hour expiry

  // Store UUID in KV Storage
  await MY_KV_NAMESPACE.put(uuid, JSON.stringify({
    proxy: proxy,
    expiry: expiry
  }));

  return new Response(`Generated UUID: ${uuid}`, { status: 200 });
}

// Validate UUID from KV Storage
async function validateUUID(uuid) {
  ...
  const data = await MY_KV_NAMESPACE.get(uuid, { type: "json" });
  ...
  if (config.expiry < currentTime) {
    return new Response("UUID expired!", { status: 403 });
  }

  return new Response(`UUID is valid! Proxy: ${config.proxy}`, { status: 200 });
}
